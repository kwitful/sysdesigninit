...
import { createInitialState, isBusy, type AppState } from "./state.js";
...
async function refreshDocs(): Promise<void> {
  try {
    ...
    if (!state.selectedFile) {
      ...
      if (first) await loadDoc(first.name);
    } else if (state.selectedFile) {
      const still = data.files.find((f) => f.name === state.selectedFile && f.ready);
      if (still && !state.docHtml) await loadDoc(still.name);
    }
  } catch (err) {
...
async function tick(): Promise<void> {
  if (!state.sessionId || state.browsingWorkspace) {
    if (!isBusy(state.phase)) stopPolling();
    return;
  }
  try {
